...
{
    return (float2(pixel_coord) + float2(0.5f, 0.5f)) * field_info.dx;
}
...
{
    return float2(pos.x * field_info.inv_res_plus_one, pos.y * field_info.inv_res) * field_info.rdx;
}

float2 get_edge_v_uv_from_mac_pos(float2 pos, FluidFieldInfo field_info)
{
    return float2(pos.x * field_info.inv_res, pos.y * field_info.inv_res_plus_one) * field_info.rdx;
}
...
float2 sample_vel_at_pos(Texture2D<float> vel_field_u, Texture2D<float> vel_field_v, SamplerState field_sampler, float2 pos, FluidFieldInfo field_info)
{
    ...
    bool is_out_of_range = pos.x < 0.0f || pos.x > field_info.resolution || pos.y < 0.0f || pos.y > field_info.resolution;
