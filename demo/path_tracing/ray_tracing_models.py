...
class Hittable_list:
    ...
    def hit(self, ray, t_min=0.001, t_max=10e8):
        ...
        for index in ti.static(range(len(self.objects))):
            (
                ...
                material_tmp,
                color_tmp,
            ) = self.objects[index].hit(ray, t_min, closest_t)
            if is_hit_tmp:
                closest_t = root_tmp
                is_hit = is_hit_tmp
    ...
    def hit_shadow(self, ray, t_min=0.001, t_max=10e8):
        ...
        if (
            is_hit_source_temp
            and (not is_hitted_non_dielectric)
            and hitted_dielectric_num == 0
        ):
            is_hit_source = True
...
class Camera:
    ...
    def reset(self):
        ...
        self.cam_lower_left_corner[None] = (
            self.cam_origin[None] - half_width * u - half_height * v - w
        )
