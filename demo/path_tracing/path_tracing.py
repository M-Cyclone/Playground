...
import numpy as np
import argparse
from ray_tracing_models import (
    Ray,
    Camera,
    Hittable_list,
    Sphere,
            ...
            if material == 0:
                ...
                # Metal and Fuzz Metal
                    ...
                    scattered_direction = reflect(
                        scattered_direction.normalized(), hit_point_normal
                    )
                    if sample_on_unit_sphere_surface:
                ...
                # Dielectric
                    ...
                    cos_theta = ti.min(
                        -scattered_direction.normalized().dot(hit_point_normal), 1.0
                    )
                    sin_theta = ti.sqrt(1 - cos_theta * cos_theta)
                    # total internal reflection
                    if (
                        refraction_ratio * sin_theta > 1.0
                        or reflectance(cos_theta, refraction_ratio) > ti.random()
                    ):
                        scattered_direction = reflect(
                            scattered_direction.normalized(), hit_point_normal
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="samples_per_pixel  (default: 4)",
    )
    parser.add_argument(
        "--samples_in_unit_sphere",
        action="store_true",
        help="whether sample in a unit sphere",
    )
    ...
    scene.add(
        Sphere(
            center=ti.Vector([0, 5.4, -1]),
            radius=3.0,
            material=0,
            color=ti.Vector([10.0, 10.0, 10.0]),
        )
    )
    # Ground
    scene.add(
        Sphere(
            center=ti.Vector([0, -100.5, -1]),
            radius=100.0,
            material=1,
    ...
    scene.add(
        Sphere(
            center=ti.Vector([-101.5, 0, -1]),
            radius=100.0,
            material=1,
            color=ti.Vector([0.6, 0.0, 0.0]),
        )
    )
    # left wall
    scene.add(
    ...
    scene.add(
        Sphere(
            center=ti.Vector([-0.8, 0.2, -1]),
            radius=0.7,
            material=2,
            color=ti.Vector([0.6, 0.8, 0.8]),
        )
    )
    # Glass ball
    scene.add(
        Sphere(
            center=ti.Vector([0.7, 0, -0.5]),
            radius=0.5,
            material=3,
    ...
    scene.add(
        Sphere(
            center=ti.Vector([0.6, -0.3, -2.0]),
            radius=0.2,
            material=4,
            color=ti.Vector([0.8, 0.6, 0.2]),
        )
    )
